define(['angularAMD', 'util'], function (angularAMD) {
  'use strict';
  angularAMD.factory('dlGridFactory', ['$compile', '$http', '$templateCache', function ($compile, $http, $templateCache) {
    ...
    Grid.prototype.buildHead = function () {
      ...
      $.each(col, function (i, item) {
        item.width = self.cellWidthPercent ? Math.ceil(parseFloat(item.width) * 0.01 * bodyRightWidth) : item.width;
